...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    MashToken public mash;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public mashPerBlock;
    // Bonus muliplier for early mash makers.
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 mashReward = multiplier.mul(mashPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(mashReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mashReward = multiplier.mul(mashPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        mash.mint(devaddr, mashReward.div(10));
        mash.mint(address(this), mashReward);
        pool.accEggPerShare = pool.accEggPerShare.add(mashReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe mash transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 mashBal = mash.balanceOf(address(this));
        if (_amount > mashBal) {
            mash.transfer(_to, mashBal);
        } else {
            mash.transfer(_to, _amount);
        }
    }
